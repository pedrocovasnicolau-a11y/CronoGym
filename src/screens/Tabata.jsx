// Tabata / HIIT — configuración + ejecución
...
const PRESETS = [
  {
    n: 'Clásico',
    c: { prep: 10, work: 20, rest: 10, rounds: 8, sets: 1, setRest: 60, cooldown: 0 },
  },
  { n: 'HIIT', c: { prep: 10, work: 40, rest: 20, rounds: 6, sets: 3, setRest: 60, cooldown: 30 } },
  {
    n: 'EMOM 10',
    c: { prep: 0, work: 45, rest: 15, rounds: 10, sets: 1, setRest: 0, cooldown: 0 },
  },
  {
    n: 'Quemar',
    c: { prep: 10, work: 30, rest: 10, rounds: 10, sets: 2, setRest: 90, cooldown: 60 },
  },
];
  ...
  for (let s = 1; s <= cfg.sets; s++) {
    for (let r = 1; r <= cfg.rounds; r++) {
      ...
      if (r < cfg.rounds && cfg.rest > 0)
        arr.push({ type: 'rest', dur: cfg.rest, round: r, set: s });
    }
    if (s < cfg.sets && cfg.setRest > 0)
      arr.push({ type: 'setRest', dur: cfg.setRest, round: 0, set: s });
  }
  if (cfg.cooldown > 0) arr.push({ type: 'cooldown', dur: cfg.cooldown, round: 0, set: cfg.sets });
  return arr;
}

export function TabataConfigScreen({ back, onStart, cfg, setCfg, onTab }) {
  ...
  const stepper = (onMinus, onPlus) => (
    <div style={{ display: 'flex', gap: 4 }}>
      <div className="press picker-btn" style={{ width: 26, height: 26 }} onClick={onMinus}>
        <IconMinus size={12} />
      </div>
      <div className="press picker-btn" style={{ width: 26, height: 26 }} onClick={onPlus}>
        <IconPlus size={12} />
      </div>
    </div>
  );
  ...
  const pickerValue =
    pickerTarget === 'work'
      ...
      : pickerTarget === 'rest'
        ? cfg.rest
        : pickerTarget === 'setRest'
          ? cfg.setRest
          : pickerTarget === 'prep'
            ? cfg.prep
            : pickerTarget === 'cooldown'
              ? cfg.cooldown
              : 0;
  ...
  const handlePickerConfirm = (v) => {
    if (pickerTarget === 'work') setCfg((c) => ({ ...c, work: Math.max(5, Math.min(600, v)) }));
      ...
      setCfg((c) => ({ ...c, rest: Math.max(0, Math.min(600, v)) }));
    else if (pickerTarget === 'setRest')
      setCfg((c) => ({ ...c, setRest: Math.max(0, Math.min(600, v)) }));
    else if (pickerTarget === 'prep')
      setCfg((c) => ({ ...c, prep: Math.max(0, Math.min(120, v)) }));
    else if (pickerTarget === 'cooldown')
      setCfg((c) => ({ ...c, cooldown: Math.max(0, Math.min(300, v)) }));
    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div className="eyebrow" style={{ color: 'var(--work)', fontSize: 9 }}>
                Trabajo
              </div>
              {stepper(
                (e) => {
            ...
            <div className="digits" style={{ fontSize: 24, color: 'var(--text)' }}>
              {fmtMMSS(cfg.work)}
            </div>
            <div
              style={{
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div className="eyebrow" style={{ color: 'var(--rest)', fontSize: 9 }}>
                Descanso
              </div>
              {stepper(
                (e) => {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 }}>
          ...
          {accentRow(
            'Recuperación final',
            fmtMMSS(cfg.cooldown),
            (e) => {
              e.stopPropagation();
              setCfg((c) => ({ ...c, cooldown: Math.max(0, c.cooldown - 15) }));
            },
            (e) => {
              e.stopPropagation();
              setCfg((c) => ({ ...c, cooldown: Math.min(300, c.cooldown + 15) }));
            },
...
const PHASE_LABEL = {
  ...
  work: 'TRABAJO',
  rest: 'Descanso',
  setRest: 'Descanso largo',
  cooldown: 'Recuperación',
  done: '¡COMPLETADO!',
};
const NEXT_LABEL = { prep: 'PREP', work: 'TRAB', rest: 'DESC', setRest: 'PAUSA', cooldown: 'COOL' };

export function TabataRunScreen({ back, cfg, onTab, settings }) {
  ...
  const {
    ...
    onPhaseEnter: (_, ph) => {
      const t =
        ph.type === 'work'
          ? 'work'
          : ph.type === 'rest' || ph.type === 'setRest'
            ? 'rest'
            : 'alert';
      triggerAlert(settings, t);
    },
    onComplete: () => triggerAlert(settings, 'end'),
  });

  const cur = done ? { type: 'done', dur: 0, round: 0, set: cfg.sets } : phase;
  const color = PHASE_COLOR[cur.type];
  const label = PHASE_LABEL[cur.type];
  const pct = cur.dur > 0 ? (1 - remaining / cur.dur) * 100 : 100;

  const workRoundNum = cur.type === 'work' || cur.type === 'rest' ? cur.round : 0;
  const roundPct = done
    ? 100
    : cfg.rounds > 0
      ? cur.type === 'rest'
        ? Math.min(100, (cur.round / cfg.rounds) * 100)
  ...
  const nextLabel = list[index + 1] ? NEXT_LABEL[list[index + 1].type] : '—';

  return (
    <>
      <TopBar title="Tabata · En marcha" onBack={back} />
      <div
        className={`phase-bg-${color}`}
        style={{
          flex: 1,
          display: 'flex',
          flexDirection: 'column',
      ...
      >
        ...
        <div style={{ padding: '0 0 12px' }}>
          <ProgressBar pct={pct} color={`var(--${color})`} />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 12 }}>
          ...
          >
            ...
            <div className="digits" style={{ fontSize: 18, color: 'var(--text-dim)' }}>
              {nextLabel}
            </div>
          </div>
        </div>
